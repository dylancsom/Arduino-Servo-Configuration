...
// Wrist Servo (Custom pulse range for 270°)
...
// Delay between each incremental movement (in milliseconds) The higher the number, the slower the movement.
  ...
  if (Serial.available() > 0) {
    ...
    switch (command) {
      ...
      case 'w': // Wrist (180° servo)
        if (angle >= 0 && angle <= 180) {  // Change to 180° limit
          pulseWidth = map(angle, 0, 180, minPulse_wrist, maxPulse_wrist);
      ...
      case 'b': // Base (270° servo)
        if (angle >= 0 && angle <= 270) {
          pulseWidth = map(angle, 0, 270, minPulse_base, maxPulse_base);
